<template>
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-indigo-600 px-6 pb-4">
                ...
                <nav class="flex flex-1 flex-col">
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    ...
                    <!-- teams -->
                    <li>
                      <div class="text-xs font-semibold leading-6 text-indigo-200">Ma`lumotlar</div>
    ...
    <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
        <form class="relative flex flex-1" action="#" method="GET">
          ...
          <input id="search-field" class="block h-full w-full border-0 py-0 pl-8 pr-0 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm" placeholder="Qidirish..." type="search" name="search" />
        </form>
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                <MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
                  <button @click="item.name == 'Chiqish' ? exit() : profilePage()" 
                  :class="[active ? 'bg-gray-50' : '', 'block w-full px-3 py-1 text-sm leading-6 text-gray-900']">
                    {{ item.name }}
                  </button>
                </MenuItem>
              </MenuItems>
    ...
    <logoutConfirm @response="confirmLogout" />
</template>
<script>
...
export default {
    components: {
        ...
        BellIcon,
        XMarkIcon,
        ChevronDownIcon, MagnifyingGlassIcon,Dialog,
        DialogPanel,
        Cog6ToothIcon,
        Menu,
        MenuButton,
  ...
  data: () => ({
        ...
        sidebarOpen: false,
        teams: [
          { id: 1, name: 'Maqolalar', initial: 'M', current: true },
          { id: 2, name: `E'lonlar`, initial: 'E', current: false },
          { id: 3, name: 'Statistika', initial: 'S', current: false },
      ],
        userNavigation: [
          { name: 'Akkaunt' },
          { name: 'Chiqish' },
    ...
    methods: {
        ...
        exit() {
          this.$store.commit('setLogoutConfirm', true)
        }
    }
}
